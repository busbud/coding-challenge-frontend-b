...
export class BusbudService {
  ...
  fetchResults(
    ...
    // build Url
    this._configUrl =
      this._configUrl + `${origin}/${destination}/${dateAndTime}/?${adult}/?${child}/?${senior}/?${lang}/?${currency}`;

    this.http.get(this._configUrl, this.httpOptions).subscribe(initialResponse => {
      this.departures = [];
  ...
  updateResults(response: any) {
    ...
    departures.forEach((resultItem: { [x: string]: any }) => {
      ...
      let operator = _.filter(operators, (op: any) => {
        return op.id === resultItem.operator_id;
      });
