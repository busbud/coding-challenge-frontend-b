...
it('produces trips from API response', done => {
  ...
  const store = null;
  const action$ = ActionsObservable.of({ type: POLL_TRIPS });

  pollTripsEpic(action$, store, { ajax, delayBetweenPollsInMs: 0 })
    .toArray()
    .subscribe(actions => {
      expect(actions).toEqual([{ type: POLL_TRIPS_SUCCESS, apiResponse }]);
      done();
    });
});
...
it('raises an error if something went wrong with the API', done => {
  ...
  pollTripsEpic(action$, store, { ajax, delayBetweenPollsInMs: 0 })
    .toArray()
    .subscribe(actions => {
      expect(actions).toEqual([{ type: POLL_TRIPS_ERROR }]);
      done();
    });
});
